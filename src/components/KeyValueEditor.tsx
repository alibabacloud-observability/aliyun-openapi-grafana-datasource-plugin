...
export const KeyValueEditor = ({ columns, values, onChange, addRowLabel, onBlur }: Props) => {
  ...
  const translateParamsToArr = (params: any): Array<Pair<string, string>> => {
    const keys = Object.keys(params);
    return keys.map((key) => ([key, params[key]]));
  }

  const translateArrToParams = (arr: Array<Pair<string, string>>): any => {
    const params: any = {};
    arr.forEach(([key, val]) => {
      params[key] = val;
    });
  ...
  const valuesArr = translateParamsToArr(values);

  const updateCell = (colIdx: number, rowIdx: number, value: string) => {
    onChange( translateArrToParams(
      valuesArr.map(([key, val], idx) => {
        if (rowIdx === idx) {
          if (colIdx === 0) {
            return [value, val];
          } else if (colIdx === 1) {
  ...
  const addRow = (i: number) => {
    onChange(translateArrToParams([...valuesArr.slice(0, i + 1), ['', ''], ...valuesArr.slice(i + 1)]));
  };

  const removeRow = (i: number) => {
    onChange(translateArrToParams([...valuesArr.slice(0, i), ...valuesArr.slice(i + 1)]));
  };
